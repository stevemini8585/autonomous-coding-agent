...
try:
    ...
    check_against_baseline = None


def test_check_against_baseline_callable():
    """check_against_baseline 호출 가능 여부(스모크)"""
    assert callable(check_against_baseline)
...
def test_new_findings_callable():
    """new_findings 호출 가능 여부(스모크)"""
    assert callable(new_findings)
