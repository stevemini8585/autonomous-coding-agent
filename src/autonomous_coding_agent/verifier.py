...
from .models import VerificationResult, PlanStep, StepType, StepStatus
...
class Verifier:
    ...
    def verify_step(self, step: PlanStep, project_files: List[str]) -> VerificationResult:
        ...
        log.info(f"  검증 완료: {'PASS' if result.passed else 'FAIL'} ({result.duration_seconds:.1f}초)")
    ...
    def _run_command(self, command: str, name: str, files: Optional[List[str]] = None) -> Dict[str, Any]:
        """명령어 실행 및 결과 파싱"""
...
def run_verification(workspace: Path, step: PlanStep, project_files: List[str]) -> VerificationResult:
    """검증 헬퍼"""
