...
class GitHubClient:
    ...
    def _run_json(self, cmd: str) -> Optional[Dict]:
        """JSON 출력 명령 실행"""
        # 명령에 이미 --json이 있는지 확인
        if "--json" not in cmd:
            cmd = cmd + " --json"
    ...
    def list_issues(self, state: str = "open", labels: Optional[List[str]] = None, limit: int = 20) -> List[GitHubIssue]:
        """이슈 목록 조회"""
    ...
    def create_issue(self, title: str, body: str, labels: Optional[List[str]] = None, assignees: Optional[List[str]] = None) -> Optional[int]:
        """이슈 생성"""
    ...
    def create_pr(self, title: str, body: str, head: str, base: str = "main", draft: bool = False) -> Optional[int]:
        """PR 생성"""
    ...
    def get_pr(self, number: int) -> Optional[GitHubPR]:
        """PR 조회"""
        data = self._run_json(
    ...
    def list_prs(self, state: str = "open", base: Optional[str] = None, limit: int = 20) -> List[GitHubPR]:
        """PR 목록 조회"""
